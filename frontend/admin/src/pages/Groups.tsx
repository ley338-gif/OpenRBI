...
export function Groups() {
  ...
  return <div className="page">
    ...
    <section className="card groups-management-card">
      <div className="groups-filter-bar"><label className="users-search"><Icons.Search /><input value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Search by group name or description…" aria-label="Search groups" /></label><select value={policyId} onChange={(e) => { setPolicyId(e.target.value); setPage(1); }} aria-label="Filter by policy"><option value="">All policies</option>{policies.map((policy) => <option value={policy.id} key={policy.id}>{policy.name}</option>)}</select>{hasFilters && <button className="btn btn-ghost btn-sm" onClick={clearFilters}>Clear filters</button>}</div>
