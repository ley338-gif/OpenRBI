...
export function Groups() {
  ...
  return <div className="page">
    ...
    <section className="card groups-management-card">
      ...
      {!data ? <div className="dashboard-skeleton"><span /><span /><span /><span /></div> : data.items.length === 0 ? <EmptyState icon={<Icons.Groups />} title={hasFilters ? "No matching groups" : "No groups yet"}>{hasFilters ? <><p>Try changing your search or filters.</p><button className="btn btn-secondary btn-sm" onClick={clearFilters}>Clear filters</button></> : <><p>Create your first group to organize users and assign access policies.</p><button className="btn btn-primary btn-sm" onClick={() => setShowCreate(true)}>Create Group</button></>}</EmptyState> : <div className="table-wrap"><table className="data-table groups-table"><thead><tr><th><button onClick={() => sort("name")}>Group {sortBy === "name" ? (sortDir === "asc" ? "↑" : "↓") : ""}</button></th><th><button onClick={() => sort("members")}>Members {sortBy === "members" ? (sortDir === "asc" ? "↑" : "↓") : ""}</button></th><th>Policies</th><th className="groups-secondary"><button onClick={() => sort("created_at")}>Created {sortBy === "created_at" ? (sortDir === "asc" ? "↑" : "↓") : ""}</button></th><th>Actions</th></tr></thead><tbody>{data.items.map((group) => <tr key={group.id}><td><div className="identity-cell"><span className="identity-avatar"><Icons.Groups /></span><div className="identity-main"><button type="button" className="identity-title identity-title-button" onClick={() => setSelectedGroup(group)}>{group.name}</button>{group.description && <span className="identity-meta">{group.description}</span>}</div></div></td><td><span className="table-count">{group.member_count}</span> <span className="text-muted">members</span></td><td><PolicyPills policies={group.policies} /></td><td className="groups-secondary">{formatDateTime(group.created_at)}</td><td><div className="row-actions"><Link className="icon-btn" to={`/groups/${group.id}`} title="Manage policies" aria-label={`Manage policies for ${group.name}`}><Icons.Shield /></Link><button className="icon-btn danger" onClick={() => setPendingDelete(group)} title="Delete group" aria-label={`Delete ${group.name}`}><Icons.Trash /></button></div></td></tr>)}</tbody></table></div>}
    ...
    {selectedGroup && <div className="modal-overlay" onClick={() => setSelectedGroup(null)}><section className="modal group-details-modal" role="dialog" aria-modal="true" aria-labelledby="group-details-title" onClick={(event) => event.stopPropagation()}><div className="section-header"><div><h2 id="group-details-title">{selectedGroup.name}</h2><p className="text-muted">{selectedGroup.description || "No description provided."}</p></div><button type="button" className="icon-btn" onClick={() => setSelectedGroup(null)} title="Close" aria-label="Close group details">×</button></div><div className="group-details-grid"><div><span>Members</span><strong>{selectedGroup.member_count}</strong></div><div><span>Created</span><strong>{formatDateTime(selectedGroup.created_at)}</strong></div><div className="group-details-policies"><span>Assigned policies</span><PolicyPills policies={selectedGroup.policies} /></div></div><div className="modal-actions"><button type="button" className="btn btn-secondary" onClick={() => setSelectedGroup(null)}>Close</button><Link className="btn btn-secondary" to={`/groups/${selectedGroup.id}`}><Icons.Shield /> Manage policies</Link><button type="button" className="btn btn-danger" onClick={() => { setPendingDelete(selectedGroup); setSelectedGroup(null); }}><Icons.Trash /> Delete group</button></div></section></div>}
