...
export function Policies() {
  ...
  function load() {
    setError(null);
    adminApi.listPolicies({ search: search || undefined, policy_type: type || undefined, status_filter: status || undefined, usage: usage || undefined, archived: showArchived ? "true" : undefined, sort_by: sortBy, sort_dir: sortDir, offset: (page - 1) * pageSize, limit: pageSize }).then(setData).catch(() => setError("Could not load policies."));
  }
  useEffect(load, [search, type, status, usage, showArchived, sortBy, sortDir, page, pageSize]);

  function clearFilters() { setSearch(""); setType(""); setStatus(""); setUsage(""); setShowArchived(false); setPage(1); }
  async function restore(policy: PolicySummaryDto) {
    try { await adminApi.restorePolicy(policy.id); notify(`Policy "${policy.name}" restored`); load(); }
    catch (err) { notify(err instanceof Error ? err.message : "Could not restore this policy", "error"); }
  }
  ...
  const hasFilters = !!(search || type || status || usage || showArchived);
