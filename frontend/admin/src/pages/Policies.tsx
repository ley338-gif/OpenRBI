...
export function Policies() {
  ...
  return <div className="page policies-page">
    ...
    <section className="card policy-console">
      <div className="policy-filters"><label className="search-input"><Icons.Search /><input value={search} placeholder="Search policy name or description…" onChange={(e) => { setSearch(e.target.value); setPage(1); }} /></label><select value={type} onChange={(e) => { setType(e.target.value); setPage(1); }}><option value="">All types</option>{POLICY_TYPES.map((item) => <option key={item}>{item}</option>)}</select><select value={status} onChange={(e) => { setStatus(e.target.value); setPage(1); }}><option value="">All statuses</option><option value="PUBLISHED">Published</option><option value="DRAFT">Draft available</option></select><select value={usage} onChange={(e) => { setUsage(e.target.value); setPage(1); }}><option value="">All usage</option><option value="IN_USE">In use</option><option value="UNASSIGNED">Unassigned</option></select><button className="btn btn-secondary btn-sm" disabled={!hasFilters} onClick={clearFilters}>Clear filters</button></div>
