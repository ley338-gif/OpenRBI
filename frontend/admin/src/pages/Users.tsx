...
export function Users() {
  ...
  return <div className="page">
    ...
    <section className="card users-management-card">
      <div className="management-filters users-filter-grid">
        <label className="search-input"><Icons.Search /><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search by username…" aria-label="Search users" /></label>
        <select className={role ? "filter-active" : ""} value={role} onChange={(e) => { setRole(e.target.value); setPage(1); }} aria-label="Filter by role"><option value="">All roles</option>{data?.roles.map((item) => <option key={item} value={item}>{item}</option>)}</select>
        <select className={groupId ? "filter-active" : ""} value={groupId} onChange={(e) => { setGroupId(e.target.value); setPage(1); }} aria-label="Filter by group"><option value="">All groups</option>{groups.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
        <select className={status ? "filter-active" : ""} value={status} onChange={(e) => { setStatus(e.target.value); setPage(1); }} aria-label="Filter by status"><option value="">All statuses</option><option value="ACTIVE">Active</option><option value="DISABLED">Disabled</option></select>
        <select className={source ? "filter-active" : ""} value={source} onChange={(e) => { setSource(e.target.value); setPage(1); }} aria-label="Filter by authentication source"><option value="">All sources</option><option value="LOCAL">Local</option><option value="LDAP">LDAP</option></select>
        <select className={mfa ? "filter-active" : ""} value={mfa} onChange={(e) => { setMfa(e.target.value); setPage(1); }} aria-label="Filter by MFA"><option value="">All MFA states</option><option value="ENABLED">MFA enabled</option><option value="NOT_ENABLED">MFA not enabled</option></select>
        <button className="btn btn-secondary btn-sm filter-reset" disabled={!hasFilters} onClick={clearFilters}>Clear filters</button>
      </div>

      {!data ? <div className="dashboard-skeleton"><span /><span /><span /><span /></div> : data.items.length === 0 ? <EmptyState title={hasFilters ? "No matching users" : "No users yet"}>{hasFilters ? <><p>Try changing your search or filters.</p><button className="btn btn-secondary btn-sm" onClick={clearFilters}>Clear filters</button></> : "Create a local user or configure LDAP to get started."}</EmptyState> : <div className="table-wrap"><table className="data-table users-table"><thead><tr><th><button onClick={() => sort("username")}>User {sortBy === "username" ? (sortDir === "asc" ? "↑" : "↓") : ""}</button></th><th className="users-secondary">Source</th><th><button onClick={() => sort("role")}>Role {sortBy === "role" ? (sortDir === "asc" ? "↑" : "↓") : ""}</button></th><th className="users-secondary">Groups</th><th><button onClick={() => sort("status")}>Status {sortBy === "status" ? (sortDir === "asc" ? "↑" : "↓") : ""}</button></th><th>MFA</th><th className="users-secondary">Last login</th><th>Actions</th></tr></thead><tbody>{data.items.map((item) => <tr key={item.id}><td><div className="identity-cell"><span className="identity-avatar">{item.username.slice(0, 2).toUpperCase()}</span><div className="identity-main"><Link className="identity-title" to={`/users/${item.id}`}>{item.username}</Link><span className="identity-meta">{item.auth_source === "LOCAL" ? "Local OpenRBI account" : "Directory-managed account"}</span></div></div></td><td className="users-secondary"><span className="identity-source">{item.auth_source}</span></td><td><span className={`role-pill role-${item.role.toLowerCase()}`}>{item.role}</span></td><td className="users-secondary"><GroupPills groups={item.groups} /></td><td><StatusBadge value={item.is_active ? "ACTIVE" : "DISABLED"} /></td><td><span className="mfa-cell"><Icons.Shield /> {item.mfa_enabled ? "ENABLED" : "NOT ENABLED"}</span></td><td className="users-secondary">{item.last_login_at ? formatDateTime(item.last_login_at) : "Never"}</td><td><div className="row-actions"><Link className="icon-btn" to={`/users/${item.id}`} title="Edit user" aria-label={`Edit ${item.username}`}><Icons.Pencil /></Link><Link className="icon-btn" to={`/users/${item.id}`} title="Security and sessions" aria-label={`Security settings for ${item.username}`}><Icons.Shield /></Link><button className={`icon-btn${item.is_active ? " danger" : ""}`} title={item.is_active ? "Disable user" : "Enable user"} aria-label={`${item.is_active ? "Disable" : "Enable"} ${item.username}`} disabled={item.id === currentUser?.id} onClick={() => setPendingAction(item)}><Icons.Power /></button></div></td></tr>)}</tbody></table></div>}
