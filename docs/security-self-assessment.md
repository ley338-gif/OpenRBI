...
| **GAP — sandbox-to-sandbox isolation between two concurrently live sessions is not covered by any automated test.** `CHANGELOG.md`'s Phase 9 entry describes this as manually verified once during that phase ("sandbox-to-sandbox blocked"), but no `backend/tests/` or `scripts/run-security-tests.sh` check exercises it today. | **open gap**, tracked under Roadmap A3 | Confirmed by reading `scripts/run-security-tests.sh` in full: its three checks are public-reachable, Postgres-blocked, and metadata-blocked — all sandbox-to-*infrastructure*, none sandbox-to-*sandbox*. Recommend closing this as part of A3's remaining scope (see Roadmap). |
| **GAP — no automated DNS-rebinding-specific test.** `docs/security-model.md` reasons that IP-based egress blocking defeats rebinding at connection time (a resolved-then-dialed blocked address is dropped the same as a hardcoded one), which is a sound argument, but it is not backed by a test that actually forces a rebinding scenario (e.g. resolving an allowed-looking hostname to a blocked IP and confirming the connection still fails). | **open gap**, tracked under Roadmap A3 | Same review of `scripts/run-security-tests.sh` and `backend/tests/` — no such test exists today. |
| Two parallel sessions of different users cannot see or act on each other's data (API/DB level) | confirmed | Covered by V4's ownership tests above (`test_user_cannot_see_another_users_session`, etc.) — this is the data-plane half of session isolation; the network-plane half (sandbox-to-sandbox) is the gap noted directly above. |
...
1. **Sandbox-to-sandbox network isolation has no automated test** — real for the platform's core isolation promise, currently verified only by a one-time manual check from Phase 9. Recommend as the top item for Roadmap A3.
2. **No automated DNS-rebinding-specific test** — the underlying IP-based mitigation is sound and documented, but untested as a distinct scenario. Second item for A3.
3. **Segmented deployment's two listener processes still share DB/Session-Agent credentials** — already honestly documented in ADR 0011, not new, but restated here so this assessment doesn't read as implying it's resolved.
4. **Session cookie `Secure` flag depends on an operator correctly setting `OPENRBI_ENVIRONMENT=production`** — not independently enforced; a deployment that skips this step silently loses the protection with no startup warning.
