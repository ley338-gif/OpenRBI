...
What this profile **now optionally gives you**, opt-in per [ADR 0025](adr/0025-segmented-credential-scoping.md) — genuinely scoped credentials, not just separate routes:

- **Separate Postgres roles.** Run `./scripts/provision-segmented-db-roles.sh` once, after the first `./scripts/deploy.sh` run has migrated the database (see [Installation](#installation) above) — it provisions `openrbi_user`/`openrbi_admin` roles and their grants directly against the running database; it is **not** a `docker-entrypoint-initdb.d` script and needs no compose changes, since it must run after migrations have created the schema, not at Postgres's own first boot. `openrbi_user` has no access at all to `ldap_configs`, `policies`/`policy_versions`/`group_policies`/`file_policy_rules`, `groups`/`user_groups`, `incidents`, `worker_metric_samples`, or `users.role_id`/`is_active` — a compromised `backend-user` cannot read another admin-only table or re-privilege any account, even via raw SQL (verified directly: a real `openrbi_user` connection gets Postgres's own `permission denied` error on each of those). Point `backend-user`/`backend-admin` at their respective role afterward by setting `OPENRBI_DATABASE_URL_USER`/`OPENRBI_DATABASE_URL_ADMIN` (see `.env.example`) and restarting both. The script is idempotent — safe to re-run after a version upgrade adds new tables, or to rotate the two role passwords. **One caveat, stated plainly**: this is incompatible with LDAP auto-provisioning through `backend-user`: `OPENRBI_LISTENER_MODE=user` refuses to start at all if `OPENRBI_DATABASE_URL_USER` and `OPENRBI_LDAP_ENABLED=true` are both set, because JIT LDAP account provisioning/role-remapping needs exactly the `role_id`/`INSERT` privilege this role is deliberately denied — run that listener as `both` instead, or don't enable LDAP for it, if you need both features together.
- **Session Agent token scoping.** Set `OPENRBI_AGENT_API_TOKEN_USER`/`OPENRBI_AGENT_API_TOKEN_ADMIN` (session-agent) matching `OPENRBI_SESSION_AGENT_API_TOKEN_USER`/`_ADMIN` (backend) instead of the single shared token — a `backend-user`-scoped token is rejected (`401`) calling the Session Agent's `isolate`/`restore`/list-all-sandboxes routes directly, even bypassing the backend's own RBAC entirely. Verify with `scripts/test-segmented-credential-scoping.sh` against a stack actually running with these variables set.
...
This requires: two reverse-proxy vhosts (one per origin, each with its own TLS certificate), `browser.openrbi.local` pointed at whatever fronts `backend-user`, `admin.openrbi.local` pointed at whatever fronts `backend-admin` (DNS or, for local evaluation, `/etc/hosts` entries on the client machine), and — per the gaps listed above — this is not yet a complete guide: separate reverse-proxy origins and operator-managed firewall/VLAN policy still need to be addressed before treating this as production-grade segmentation. Separate Postgres roles and Session Agent token scoping are no longer on that list — see [ADR 0025](adr/0025-segmented-credential-scoping.md) above — but remain opt-in and are not part of Compact's complete production guide.
...
**The model.** Every real poll of `GET /v1/nodes/self` (`session-agent/app/main.py`'s `_compute_capacity()`) reads the host's current free RAM and CPU, divides each by the same per-sandbox reservation the platform already enforces via Docker's own `--cpus`/`--memory` for every sandbox (`OPENRBI_AGENT_DEFAULT_CPU_LIMIT`, default 2.0 CPUs; `OPENRBI_AGENT_DEFAULT_RAM_LIMIT_MB`, default 2048 MB), and reports whichever resource is currently scarcer — a host can be RAM-bound one moment and CPU-bound the next. Free headroom is measured after what running sandboxes already use, so the reported `capacity` is the node's *total* slots: sessions already running plus how many more fit. Being RAM- or CPU-bound is normal (without a ceiling, one of the two always binds); the dashboard only warns when a node has had no free slot for `OPENRBI_CAPACITY_BOUND_WARNING_MINUTES`. The Admin Portal's Workers page and Worker Detail show exactly which one is currently binding and the raw numbers behind it (Roadmap B3.3), so you don't have to guess.
