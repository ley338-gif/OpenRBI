...
`scripts/seed-standard-policies.sh` creates a set of ready-to-use, published `MIME`/`SESSION`/`CLIPBOARD` policies as starting templates (`PDF Only`, `Office Documents`, `Images Only`, `Block Executables`, `Full HD`, `Low Resolution`, `No Clipboard` — see `scripts/seed-standard-policies.py` for the exact rules). It deliberately does **not** attach any of them to a group — a policy only ever takes effect once an admin explicitly attaches it to one (Admin Portal → Policies → assign to group), matching the fail-closed principle that nothing here silently changes behavior for real users. Idempotent — safe to run again after adding a new template to the script's `STANDARD_POLICIES` list.
