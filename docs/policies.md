...
Every installation comes with a set of ready-to-use, published `MIME`/`SESSION`/`CLIPBOARD` policies as starting templates (`PDF Only`, `Office Documents`, `Images Only`, `Block Executables`, `Full HD`, `Low Resolution`, `No Clipboard` — see `STANDARD_POLICIES` in `backend/app/services/standard_policies.py` for the exact rules). They are created when first-run setup completes, and `scripts/deploy.sh` adds templates introduced by later releases on every update. They are system-authored (no admin is recorded as their author) and deliberately **not** attached to any group — a policy only ever takes effect once an admin explicitly attaches it to one (Admin Portal → Policies → assign to group), matching the fail-closed principle that nothing here silently changes behavior for real users.

Each template is created **once per installation** (recorded in `system_state.seeded_policy_templates`). Renaming, editing or archiving one is permanent: an update never re-creates or overwrites it. A policy you created yourself under a template's name is kept as it is. `scripts/seed-standard-policies.sh` runs the same step by hand and is safe to run at any time.
