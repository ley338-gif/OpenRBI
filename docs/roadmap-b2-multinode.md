...
- Host-resource-aware auto-scaling of capacity (flagged in B2.3 as a
  follow-up, not silently dropped — addressed by [Roadmap B3](roadmap-b3-capacity-autoscaling.md):
  a node's reported capacity is now computed from its actual free
  CPU/RAM headroom, with admin-visible reasoning for why it's what it
  is, instead of the flat `OPENRBI_AGENT_CAPACITY` this roadmap
  shipped).
