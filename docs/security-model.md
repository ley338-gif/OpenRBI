...
- **Separate origins for cookie isolation** — this exists today only as a consequence of no explicit `Domain=` being set on the session cookie (`app/api/auth.py`); it becomes a real, automatic benefit once a User Portal and Admin Portal are actually served from different origins, but nothing forces that yet.
