...
- `browser-plane` (`172.30.0.0/24` by default, overridable per deployment since Roadmap B2.4 — `OPENRBI_BROWSER_PLANE_SUBNET`; IPv6 disabled outright rather than replicating the IPv4 blocklist for it) — every browser sandbox container, and *only* the session-agent additionally, at a pinned address (`172.30.0.2` by default). Since [ADR 0024](adr/0024-cross-host-display-relay.md) (Roadmap B2.4), session-agent — not the backend — needs this for the display relay (a plain outbound TCP connection to a sandbox's VNC port — not a privileged operation, ADR 0005 is unaffected). The backend no longer has any presence on `browser-plane` at all; it only ever talks to session-agent over `control-plane`, the same as every other Session-Agent-bound call.
...
- Allows `ESTABLISHED,RELATED` return traffic for control-plane-initiated connections, and a plain `ACCEPT` for new connections specifically from the session-agent's pinned address (`OPENRBI_AGENT_BROWSER_PLANE_IP`, default `172.30.0.2`) — otherwise indistinguishable from a sandbox by subnet alone. Every other address in `browser-plane` (every real sandbox) still cannot open a *new* connection into the control plane; only session-agent can initiate.
...
Verified end-to-end on the actual running stack (not just written and assumed correct): a container on `browser-plane` reaches the public internet (`HTTP 200`) but times out against Postgres's real container IP, the network gateway, `10.0.0.1`, and `169.254.169.254`; the display relay still gets a real VNC banner back from a live sandbox; a sandbox cannot reach another sandbox — this last one was a one-time manual check during Phase 9 itself, since promoted to an automated regression test in `scripts/run-security-tests.sh` (Roadmap Phase A / A3, closing a gap `docs/security-self-assessment.md` found). Two real bugs were caught and fixed during this original Phase 9 verification (when the backend, not session-agent, was the pinned `browser-plane` member — see [ADR 0024](adr/0024-cross-host-display-relay.md) for the Roadmap B2.4 change that moved this role to session-agent): the pinned member's own leg on `browser-plane` was being caught by the same DROP rules meant for sandboxes (fixed by pinning its address and exempting exactly that address), and the `LOG` rules were silently never firing because they were inserted *after* their matching `DROP` rule instead of before it.
