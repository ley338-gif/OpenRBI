...
- **Segmented** — an illustrative, additive `docker-compose.segmented.yml` overlay runs two instances of the *same* backend image, `backend-user` (`OPENRBI_LISTENER_MODE=user`) and `backend-admin` (`OPENRBI_LISTENER_MODE=admin`), alongside (not replacing) the base `backend` service. This is explicitly documented as a preparatory example, not a complete production guide — it does not yet add a second reverse-proxy vhost/origin (see [ADR 0011](0011-user-admin-listener-separation.md)'s "Alternatives Considered" for why that's deliberately deferred). Separate database roles and Session Agent token scoping are no longer deferred — [ADR 0025](0025-segmented-credential-scoping.md) adds them as a further, opt-in operator step (`scripts/provision-segmented-db-roles.sh` plus new `.env` variables) on top of this one.
...
- A future, fuller Segmented production guide (separate origins/vhosts, TLS per-origin, firewall guidance) remains a distinct, later piece of work — not implied as already done by this ADR. DB role scoping and Session Agent token scoping are implemented and opt-in as of [ADR 0025](0025-segmented-credential-scoping.md), but are not yet part of that complete guide either.
