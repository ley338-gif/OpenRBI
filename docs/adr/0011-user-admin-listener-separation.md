...
- The Session Agent's shared static bearer token and the single Postgres connection are still available, at the credential level, to whichever listener-mode process holds them — this ADR closes the *route-existence* boundary, not the *credential-scoping* boundary. See the "Alternatives Considered" note above; that's deliberately out of scope here. [ADR 0025](0025-segmented-credential-scoping.md) later closes this as an opt-in for Segmented deployments specifically (Compact remains one shared role/token, unaffected).
