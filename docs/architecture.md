...
This is a **logical/process-level** separation by default, not a network-level one: both modes reach the same PostgreSQL, the same Redis, and hold the same Session Agent shared token when run as separate processes with no further configuration. [ADR 0025](adr/0025-segmented-credential-scoping.md) adds an **opt-in** credential-level boundary on top for Segmented specifically — genuinely scoped Postgres roles (`openrbi_user`/`openrbi_admin`) and Session Agent tokens, enforced by Postgres and by the Session Agent itself, not just by route non-existence — but Compact and any Segmented deployment that hasn't explicitly opted in are unaffected: still one shared role, one shared token. See [ADR 0011](adr/0011-user-admin-listener-separation.md) for the route-existence boundary's own reasoning, [ADR 0012](adr/0012-compact-vs-segmented-deployment.md) for how this maps onto **Compact** (today's only shipped, complete profile) vs. the illustrative, not-yet-fully-complete **Segmented** profile (`docker-compose.segmented.yml`, still no separate reverse-proxy vhosts — see `docs/deployment.md#segmented`), and ADR 0025 for the credential scoping.
...
- **Segmented**: since Roadmap B2.4 ([ADR 0024](adr/0024-cross-host-display-relay.md)), neither `backend-user` nor `backend-admin` has any `browser-plane` network at all — the display relay terminates at each node's own Session Agent instead of the backend, so `backend-user`'s previous browser-plane membership (needed only to dial a sandbox's VNC port directly) is gone. `scripts/setup-network-isolation.sh` exempts the Session Agent's own address (`OPENRBI_AGENT_BROWSER_PLANE_IP`, default `172.30.0.2`) — a single exemption shared by Compact and Segmented alike, since both profiles use the same one Session Agent. See `docker-compose.yml`'s comments and `docs/deployment.md#segmented` for details.
...
- Per-node capacity was originally real deploy-time config (`OPENRBI_AGENT_CAPACITY` on the Session Agent, default 10, a B2 non-goal to auto-size). [Roadmap B3](roadmap-b3-capacity-autoscaling.md) picked that up directly: capacity is now computed from a node's actual free CPU/RAM headroom at poll time (whichever resource is scarcer binds), with asymmetric hysteresis (a drop applies immediately, a rise only after several consecutive polls sustain it) and admin-visible reasoning for which resource is currently binding and why. `OPENRBI_AGENT_CAPACITY` survives only as an optional ceiling on that computed value, unset (uncapped) by default.
...
- The noVNC display relay (Roadmap B2.4, [ADR 0024](adr/0024-cross-host-display-relay.md)) terminates at each node's own Session Agent instead of the backend dialing a sandbox's VNC port directly — the only way the relay can keep working once a node is genuinely on a different host. The backend has no `browser-plane` network presence at all any more; only the Session Agent does, at a per-deployment-configurable pinned address (`OPENRBI_AGENT_BROWSER_PLANE_IP`). Every access control the relay enforces (session ownership, status, Origin, real-time clipboard-policy filtering) still runs entirely on the backend, unchanged — the relay hop only ever carries bytes already cleared by those checks. **Not yet verified against a genuine second host** — see the roadmap doc's B2.4 entry for the stated verification gap.
