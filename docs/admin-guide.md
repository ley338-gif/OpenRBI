...
**Group Detail** (`/groups/{id}`, `GET /admin/groups/{id}`): opened via a group's "Manage policies" icon on the overview. Shows member count and creation date, and — the actual point of this page — an **Assigned policies** panel: search-and-add a published or draft policy by name (a directory-service-style "add this policy to this group" widget, `AttachList`, shared with the reverse direction below), with each attached row offering its own **Remove**. This is real attach/detach against `POST`/`DELETE /admin/policies/{policy_id}/groups/{group_id}` — endpoints that existed since the policy engine's own Phase 12 work but, until this page, had no UI calling them at all.

**Policy Detail** gained the mirror-image **Assigned groups** panel using the same `AttachList` widget, so a group can be attached to a policy from either side — matching Active Directory's "add this group to this policy" / "add this policy to this group" pattern the admin already expects. Group membership itself (which users belong to a group) is still managed only from user creation or `PUT /admin/users/{id}/groups`; there is no member-management UI on Group Detail. The data model still has no group-to-role relationship, LDAP group synchronization/source field, or group `updated_at`, so the UI does not claim those capabilities or display invented values.
