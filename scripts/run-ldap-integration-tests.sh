...
wait_for_ldap() {
    ...
    for i in $(seq 1 30); do
        # A successful ldapsearch against localhost inside the LDAP
        # container only proves the container's own local admin socket is
        # up. It does NOT prove the LDAPS TCP listener is actually
        # accepting connections over the docker network yet — there's a
        # real, reproducible-once gap between those two points (seen in
